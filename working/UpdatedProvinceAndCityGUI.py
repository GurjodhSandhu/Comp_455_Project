...
province_options = ['Alberta', 'British Columbia', 'Manitoba', 'New Brunswick', 'Newfoundland and Labrador', 'Nova Scotia', 'Ontario', 'Prince Edward Island', 'Quebec', 'Saskatchewan']
...
def estimate_price():
    ...
    if len(X) >= 2:
        ...
        result_label.config(text="Estimated price: $" + str(output))
    else:
        result_label.config(text="Not enough data to estimate price.")

estimate_button = Button(window, text="Estimate Price", command=estimate_price)
